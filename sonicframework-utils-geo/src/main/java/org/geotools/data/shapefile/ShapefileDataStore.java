...
public class ShapefileDataStore extends ContentDataStore implements FileDataStore {
    ...
    public static final String SONIC_SCALES = "sonic.scales";
    ...
    private static Logger logger = org.geotools.util.logging.Logging.getLogger(ShapefileDataStore.class);
    
    static {
    	logger.info("load local ShapefileDataStore");
    }
    ...
    protected static DbaseFileHeader createDbaseHeader(SimpleFeatureType featureType)
        ...
        for (int i = 0, ii = featureType.getAttributeCount(); i < ii; i++) {
            ...
            if ((colType == Integer.class) || (colType == Short.class) || (colType == Byte.class)) {
                ...
                //通过自定义属性支持小数点位数,decimalCount
                Map<Object, Object> userData = type.getUserData();
                if (userData != null) {
                	d = Integer.parseInt(userData.getOrDefault(SONIC_SCALES, d).toString());
                }
