...
class ShapefileFeatureSource extends ContentFeatureSource {
	...
	private static Logger logger = org.geotools.util.logging.Logging.getLogger(ShapefileFeatureSource.class);

	static {
    	logger.info("load local ShapefileFeatureSource");
    }
        ...
        try {
            ...
            if (dbf != null) {
                ...
                for (int i = 0, ii = header.getNumFields(); i < ii; i++) {
                    ...
                    int decimalCount = header.getFieldDecimalCount(i);
                    if(decimalCount > 0){
                    	build.addUserData(ShapefileDataStore.SONIC_SCALES, decimalCount);
                    }
