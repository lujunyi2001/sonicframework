...
public class DbaseFileHeader {
    ...
    private static Logger logger = org.geotools.util.logging.Logging.getLogger(DbaseFileHeader.class);

    static {
    	logger.info("load local DbaseFileHeader");
    }
    /**
    ...
    public void writeHeader(WritableByteChannel out) throws IOException {
        ...
        try {
            ...
            int tempOffset = 0;
            String charset = DbfCharsetContextHolder.get();
            DbaseField field = null;
            for (int i = 0; i < fields.length; i++) {
            	field = fields[i];
...
//                }
            	int j = 0;
//            	byte[] fullByte1 = new String(field.fieldName.getBytes("ISO-8859-1"), "UTF-8")  
//            	        .getBytes("GBK");  
//            	j = fullByte1.length;
//            	for (int k = 0; k < fullByte1.length; k++) {
//            		buffer.put(fullByte1[k]);
//				}
            	
                for(int counter = 0, len = field.fieldName.length(); j < 11 && counter < len; counter++){
                    char cha = field.fieldName.charAt(counter);
    ...
    /*public void writeHeader(WritableByteChannel out) throws IOException {
        ...
        try {
          ...
          for (DbaseField field : fields) {
            ...
            // write the field name
            int j = 0;
            for (int counter = 0; j < 11 && counter < field.fieldName.length(); counter++) {
              char cha = field.fieldName.charAt(counter);
              String str = new String(cha + "");
              byte[] bytes = str.getBytes("GBK"); //此处应改为byte[] bytes = str.getBytes(“GBK”);否则中文还是无法识别
